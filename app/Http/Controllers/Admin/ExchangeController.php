...
use App\Models\RelationSeoExchangeInfo;
...
class ExchangeController extends Controller {
    ...
    public static function list(Request $request){
        ...
        /* paginate */
        $viewPerPage        = Cookie::get('viewExchangeInfo') ?? 20;
        $params['paginate'] = $viewPerPage;
        $list               = Exchange::getList($params);
        return view('admin.exchange.list', compact('list', 'viewPerPage', 'params'));
    }
    ...
    public function view(Request $request){
        ...
        if($flagView==true){
            ...
            $prompts            = Prompt::select('*')
                                    ...
                                    ->get();
            /* tags con */
            $tags               = ExchangeTag::all();
            /* trang cha */
            $parents            = Exchange::all();
            /* type */
            $type               = !empty($itemSeo) ? 'edit' : 'create';
            $type               = $request->get('type') ?? $type;
            return view('admin.exchange.view', compact('item', 'itemSeo', 'prompts', 'parents', 'type', 'tags', 'language', 'message'));
        } else {
            return redirect()->route('admin.exchange.list');
        }
    }
    ...
    public function createAndUpdate(Request $request){
        try {
            ...
            $idExchange         = $request->get('exchange_info_id');
            $language           = $request->get('language');
            $exchangeType       = 'exchange_info';
            ...
            /* update page */
            $seo                = $this->BuildInsertUpdateModel->buildArrayTableSeo($request->all(), $exchangeType, $dataPath);
            if($action=='edit'){
            ...
            if(!empty($idSeo)){
                ...
                if($language=='vi'){
                    /* insert hoặc update exchange_info */
                    $flagShow           = !empty($request->get('flag_show'))&&$request->get('flag_show')=='on' ? 1 : 0;
                    if(empty($idExchange)){ /* check xem create exchange hay update exchange */
                        $idExchange          = Exchange::insertItem([
                            'flag_show'     => $flagShow,
                            'seo_id'        => $idSeo,
                            'sign'          => $sign,
                            'icon'          => $icon,
                        ...
                        Exchange::updateItem($idExchange, [
                            'flag_show'     => $flagShow,
                            'sign'          => $sign,
                            'icon'          => $icon,
                        ]);
                    ...
                    RelationExchangeInfoExchangeTag::select('*')
                        ->where('exchange_info_id', $idExchange)
                        ->delete();
                    if(!empty($request->get('tags'))){
                        foreach($request->get('tags') as $idExchangeTag){
                ...
                /* relation_seo_exchange_info */
                $relationSeoExchangeInfo = RelationSeoExchangeInfo::select('*')
                                        ->where('seo_id', $idSeo)
                                        ->where('exchange_info_id', $idExchange)
                                        ->first();
                if(empty($relationSeoExchangeInfo)) RelationSeoExchangeInfo::insertItem([
                    'seo_id'        => $idSeo,
                    'exchange_info_id'   => $idExchange
                ]);
                DB::commit();
                ...
                $message        = [
                    'type'      => 'success',
                    'message'   => '<strong>Thành công!</strong> Đã cập nhật Exchange!'
                ];
                ...
                if(!empty($request->get('index_google'))&&$request->get('index_google')=='on') {
                    ...
                    if($flagIndex==200){
                        $message['message'] = '<strong>Thành công!</strong> Đã cập nhật Exchange và Báo Google Index!';
                    }else {
                        $message['message'] = '<strong>Thành công!</strong> Đã cập nhật Exchange <span style="color:red;">nhưng báo Google Index lỗi</span>';
                    }
                }
            }
        ...
        return redirect()->route('admin.exchange.view', ['id' => $idExchange, 'language' => $language]);
    }
