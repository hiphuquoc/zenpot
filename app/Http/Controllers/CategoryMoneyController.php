...
class CategoryMoneyController extends Controller {
    ...
    */
    ...
    public static function buildTocContentMain($contents, $language, $add = []) {
        ...
        // thêm toccontent tùy chọn
        if(!empty($add)){
            foreach($add as $value){
                if(!empty($value['id'])&&!empty($value['name'])&&!empty($value['title'])){
                    $rowAdd = [];
                    $rowAdd['id']       = $value['id'];
                    $rowAdd['name']     = $value['name'];
                    $rowAdd['title']    = $value['title'];
                    // cho vào mảng tổng 
                    $dataTocContent[] = $rowAdd;
                }
            }
        }
    
        // Tạo nội dung TOC với view
        $xhtml = view('main.template.tocContentMain', [
            'data' => $dataTocContent,
            'language' => $language
        ])->render();
