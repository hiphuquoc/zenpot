...
class ExchangeTag extends Model {
    ...
    public function scopeWithDefaultSeoForLanguage($query, $language) {
        return $query->whereHas('seos', function ($q) use ($language) {
            $q->join('seo', 'seo.id', '=', 'relation_seo_exchange_tag.seo_id')
    ...
    public function seos() {
        return $this->hasMany(\App\Models\RelationSeoExchangeTag::class, 'exchange_tag_id', 'id');
    }

    public function exchanges(){
        return $this->hasMany(\App\Models\RelationExchangeInfoExchangeTag::class, 'exchange_tag_id', 'id');
    }
}
