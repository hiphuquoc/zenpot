<div class="repeater formBox">
    <div class="formBox_full">
        <div data-repeater-list="faqs">
            @php
                $dataFaqs = old('faqs') ?? $itemSeo->faqs;
                $dataFaqs = $dataFaqs->isNotEmpty() ? $dataFaqs : [null];
            @endphp
            @foreach($dataFaqs as $question)
                <div class="flexBox" style="flex-wrap:unset;align-items:flex-start;" data-repeater-item>
                    <div class="flexBox_item">
                        ...
                        <div class="formBox_full_item">
                            ...
                            <textarea class="form-control" name="faqs[{{ $loop->index }}][question]" rows="3">{{ $question->question ?? null }}</textarea>
                        </div>
                    </div>
                    <div class="flexBox_item">
                        <!-- One Row -->
                        <div class="formBox_full_item">
                            ...
                            <textarea class="form-control" name="faqs[{{ $loop->index }}][answer]" rows="3">{{ $question->answer ?? null }}</textarea>
                        </div>
                    </div>
